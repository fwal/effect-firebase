...
// module glue that cannot load under vitest, so the mock is fully
// self-contained instead of spreading the actual module.
vi.mock('@react-native-firebase/firestore', () => {
  ...
  const constraint = (type: string) => ({ type });
  return {
    Timestamp,
    GeoPoint,
    FieldValue,
...
describe('FirestoreService (react-native)', () => {
