...
const resolveApp = (app?: FirebaseApp): FirebaseApp => {
  ...
  try {
    ...
    throw new Error(
      'Client.layer: no Firebase app available. Pass { app } or ensure the default app is configured natively before calling Client.layer().',
      { cause: error },
    );
  }
};
 ...
 * import { Client } from '@effect-firebase/react-native';
 *
 * const layer = Client.layer({ app: getApp() });
 * ```
 */
