...
>(
  ...
  build: Effect.gen(function* () {
    ...
    for (const doc of options.docs) {
      ...
      // Document IDs become a single path segment; a separator would
      // silently move the document out of the intended collection.
      if (id.includes('/')) {
        return yield* Effect.die(
          new Error(
            `fixture(${options.collectionPath}): document ID '${id}' must not contain '/'`,
          ),
        );
      }
